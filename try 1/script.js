...
for (const cardNum of cardsPairArr) {
  ...
  card.addEventListener("click", function () {
    if (
      card.classList.contains("check") ||
      card.classList.contains("checkSuccess")
    ) {
      return;
    }
    ...
    if (firstCard === null) {
      ...
      console.log('изменил первую картинку');
      
    } else {
      secondCard = card;
      secondCard.style.backgroundImage = `URL(../img/${cardNum}.png)`;
      console.log('изменил вторую картинку');
    }
    ...
    if (
      cardsPairArr.length === document.querySelectorAll(".checkSuccess").length
    ) {
      setTimeout(() => {
